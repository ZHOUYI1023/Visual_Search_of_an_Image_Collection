...
if params.UseParallel
    [assignments, dists, varargout{1:nargout-2}] = assignDataToClustersParallel(features, centers, randState, params.DistanceMethod);
else
    [assignments, dists, varargout{1:nargout-2}] = assignDataToClustersSerial(features, centers, randState, params.DistanceMethod);
end

%--------------------------------------------------------------------------
function [assignments, dists, varargout] = assignDataToClustersSerial(features, centers, randState, distanceMethod)
if isempty(features)
    assignments = [];
    dists       = [];
    if nargout == 3
    ...
    searcher = vision.internal.Kdtree(distanceMethod);
...
%--------------------------------------------------------------------------
function [assignments, dists, varargout] = assignDataToClustersParallel(features, centers, randState, distanceMethod)
...
if isempty(pool)
    [assignments, dists] = assignDataToClustersSerial(features, centers, randState, distanceMethod);
else
    ...
    parfor n = 1:size(featuresCube,3)
        ...
        [tassignments, tdists, tisValid] = assignDataToClustersSerial(f, centers, randState, distanceMethod);
    ...
    [tassignments, tdists, tisValid] = assignDataToClustersSerial(...
        features(end-remainder+1:end,:), centers, randState, distanceMethod);
...
centers = computeClusterCenters(centerSums, counts, features);
